...
public interface MongoExpressionFactory {
    ...
    public class Impl implements MongoExpressionFactory {
        ...
        public BasicDBObject toQuery() throws ParseExpressionException {
            ...
            if (logger.isDebug())
                logger.debug("Expression is {0}.", query.toMap().toString());
                    ...
                    try {
                        intent.playback(new Tracer() {
                            ...
                            public Tracer trace(final Step step)
                                ...
                                if (logger.isDebug())
                                    logger.debug(
                                            "ExprIntent#playback, step is {0}, purpose is {1}, values is {2}.",
                                ...
                                switch (step.step()) {
                                    ...
                                    case Operator.NOT_MATCH:
                                        ...
                                        break;
                                    case MongoOperator.ALL:
                                        innerBdbo.append(step.getPurpose(), new BasicDBObject().append("$all", step.getScalar()));
                                        break;
                                    case MongoOperator.OTHER:
                                        innerBdbo.append(step.getPurpose(), step.getScalar());
                                        break;
                                    case Operator.AND:
                                        break;
